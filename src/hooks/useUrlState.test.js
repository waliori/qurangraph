...
describe("URL state round-trip", () => {
  it("decode(encode(state)) preserves all fields (incl. sets & morph filter)", () => {
    const state = {
      surah: 2, ayah: 228, mode: "lemma", precision: "strict", theme: "dark", maxBranch: 25,
  ...
  it("defaults a fresh state to a short payload and decodes back to defaults", () => {
    const enc = encodeState({ surah: 2, ayah: 1, mode: "exact", precision: "loose", theme: "light", maxBranch: 10, hideStop: true, showLoops: true, rareOnly: false });
  ...
  it("omits theme + activeLexicon when absent so a shared link can't clobber personal prefs", () => {
    ...
    const enc = encodeState({ surah: 2, ayah: 255, theme: "light", activeLexicon: "maqayis", expandedWords: new Set(["a@2:255"]) });
    const back = decodeState(enc);
    ...
    const enc2 = encodeState({ surah: 2, ayah: 255, theme: "dark", activeLexicon: "lisan" });
    const back2 = decodeState(enc2);
    expect(back2.theme).toBe("dark");
